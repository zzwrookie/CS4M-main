...
# ClearScope E5 v33b full Phase3E/Phase3G runner.
...
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/logs/clearscope_e5_v33b_full}"
...
reject_forbidden_path() {
    ...
    case "${path}" in
        ...
        *CLEARSCOPE_E5_V33B_BOUNDED*)
            echo "error: ${label} must not reference CLEARSCOPE_E5_V33B_BOUNDED: ${path}" >&2
            exit 2
            ;;
        *CLEARSCOPE_E5_v33b_bounded*)
            echo "error: ${label} must not reference CLEARSCOPE_E5_v33b_bounded: ${path}" >&2
...
preflight() {
    ...
    if [[ -z "${CLAD_DB_PASSWORD:-}" ]]; then
        echo "error: set CLAD_DB_PASSWORD before running ClearScope E5 full run" >&2
        exit 2
    fi
    if [[ "${STAGE}" != "train_word2vec" && "${STAGE}" != "all" ]]; then
        require_file "${EMBEDDER_PATH}" "ClearScope E5 v33b full residual Word2Vec embedder"
    fi
...
run_train_word2vec() {
    run_logged_command "train_word2vec" "CLEARSCOPE_E5_V33B_FULL_WORD2VEC" \
...
run_build_phase3e_artifacts() {
    run_pipeline_stage "build_phase3e_artifacts" "CLEARSCOPE_E5_V33B_FULL_ARTIFACT" \
        --sspm_train_mode build_phase3e_artifacts \
        --sspm_train_data_mode phase3e_memmap
}

run_train_phase3e_base() {
    run_pipeline_stage "train_phase3e_base" "CLEARSCOPE_E5_V33B_FULL_BASE" \
...
run_train_phase3g_head() {
    run_pipeline_stage "train_phase3g_head" "CLEARSCOPE_E5_V33B_FULL_HEAD" \
...
run_infer_full() {
    local out_tag="CLEARSCOPE_E5_V33B_FULL_PIPELINE"
